...
Create a categorical variable of age by binning age by quantiles (<25%, 25-50%, 50-75%, >75% of the distribution). First let's do it the dummy variable way, and then let's create a multi-level variable for age category.

Try to regress chol on a multiple-level categorical variable. Start by categorizing age into groups based on the quantiles of the distribution
...
Run the regression with a multi-level predictor, `agegrps`. How would you 
interpret the coefficient for as.factor(agegrps)12-17? 
```{r mod1}
