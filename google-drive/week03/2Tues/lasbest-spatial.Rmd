...
Try changing the basemap with different "provider tiles" using addProviderTiles option (e.g. CartoDB.Positron) https://leaflet-extras.github.io/leaflet-providers/preview/
...
pop.pal = colorNumeric(c('blue','brown'), domain=cities$pop)
leaflet(cities) %>% 
  addTiles() %>%
...
dat <- merge(pm25 %>% filter(date=='11/10/18'), pm25.sites, by='site_id') %>% 
  st_as_sf(coords=c('longitude','latitude'), crs=4326) # Coordinate Reference System (CRS): WGS 84 (World Geodetic System 1984)
